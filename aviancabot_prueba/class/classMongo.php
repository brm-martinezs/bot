...
class AlertMongo{
    //"mongodb://myusername:mypassword@example.com:27017/mydatabase"
    ...
    function conexion(){
        $user="brm2_us3r4pp";
        $pass="JLGhYDdMXIrI8y3n";
        $server="127.0.0.1/callaut";
        $conn='mongodb://'.$user.':'.$pass.'@'.$server;
        return $conn;
    }
    /*Funciòn para mongo */
    ...
    function getPalabras() {
        ...
        foreach ($cursor as $document) {
            //printVar($document);
            //die();
            array_push($posts, $document->nombre);
